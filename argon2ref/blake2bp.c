...
*/
static int blake2bp_init_leaf_param( ablake2b_state *S, const ablake2b_param *P )
{
  int err = ablake2b_init_param(S, P);
  S->outlen = P->inner_length;
  return err;
}

static int blake2bp_init_leaf( ablake2b_state *S, size_t outlen, size_t keylen, uint64_t offset )
{
  ablake2b_param P[1];
...
//  P->xof_length = 0;
  P->node_depth = 0;
  P->inner_length = ablake2b_OUTBYTES;
...
static int blake2bp_init_root( ablake2b_state *S, size_t outlen, size_t keylen )
{
  ablake2b_param P[1];
  P->digest_length = (uint8_t)outlen;
  P->key_length = (uint8_t)keylen;
...
int blake2bp_init_key( blake2bp_state *S, size_t outlen, const void *key, size_t keylen )
{
  ...
  if( !key || !keylen || keylen > ablake2b_KEYBYTES) return -1;
...
int blake2bp_update( blake2bp_state *S, const void *pin, size_t inlen )
{
  ...
  in += inlen - inlen % ( PARALLELISM_DEGREE * ablake2b_BLOCKBYTES);
  inlen %= PARALLELISM_DEGREE * ablake2b_BLOCKBYTES;
...
{
  uint8_t hash[PARALLELISM_DEGREE][ablake2b_BLOCKBYTES];
  size_t i;
  ...
  for( i = 0; i < PARALLELISM_DEGREE; ++i )
  {
    if( S->buflen > i * ablake2b_BLOCKBYTES)
    {
      size_t left = S->buflen - i * ablake2b_BLOCKBYTES;
...
//  uint8_t hash[PARALLELISM_DEGREE][ablake2b_OUTBYTES];
...
#endif
  {
    ...
    if( inlen__ > i * ablake2b_BLOCKBYTES)
    {
