...
enum ablake2b_constant {
    ablake2b_BLOCKBYTES = 128,
    ablake2b_OUTBYTES = 64,
    ablake2b_KEYBYTES = 64,
    ablake2b_SALTBYTES = 16,
    ablake2b_PERSONALBYTES = 16
};

#pragma pack(push, 1)
typedef struct __ablake2b_param {
    ...
    uint8_t salt[ablake2b_SALTBYTES];         /* 48 */
    uint8_t personal[ablake2b_PERSONALBYTES]; /* 64 */
} ablake2b_param;
#pragma pack(pop)

typedef struct __ablake2b_state {
    uint64_t h[8];
    uint64_t t[2];
    uint64_t f[2];
    uint8_t buf[ablake2b_BLOCKBYTES];
    unsigned buflen;
    unsigned outlen;
    uint8_t last_node;
} ablake2b_state;
...
enum {
    ...
    blake2_size_check_2 =
        1 / !!(sizeof(ablake2b_param) == sizeof(uint64_t) * CHAR_BIT)
};
...
int ablake2b_long2(void * pout, size_t outlen, const void * in, size_t inlen);
/* Simple API */
