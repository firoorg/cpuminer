...
int mtp_solver(uint32_t TheNonce, argon2_instance_t *instance,
	...
	if (instance != NULL) {
		...
		bblake2b_update(&BlakeHash, (unsigned char*)&TheNonce, sizeof(unsigned int));
		bblake2b_final(&BlakeHash, (unsigned char*)&Y);
		...
		for (int j = 1; j <= L; j++) {
			...
			bblake2b_init(&BlakeHash2);
			bblake2b_update(&BlakeHash2, (unsigned char*)&Y, sizeof(uint256));
			bblake2b_update(&BlakeHash2, blockhash_bytes, ARGON2_BLOCK_SIZE);
			bblake2b_final(&BlakeHash2, (unsigned char*)&Y);
			////////////////////////////////////////////////////////////////
			// current block
...
//		for (int i = 0; i < Number; i++) {
			blake2b_state BlakeHash;
			uint32_t ThatNonce1 = TheNonce;

			bblake2b_init(&BlakeHash);
			bblake2b_update(&BlakeHash, (unsigned char*)input, 80);
			bblake2b_update(&BlakeHash, (unsigned char*)resultMerkleRoot, 16);
			bblake2b_update(&BlakeHash, (unsigned char*)&ThatNonce1, sizeof(uint32_t));
			bblake2b_final(&BlakeHash, (unsigned char*)Y1); 

		for (uint8_t j = 1; j <= L; j++) {


			unsigned char	 TheBlock[ARGON2_BLOCK_SIZE];
			
			ij1 = ((uint32_t *)Y1)[0] % (instance->context_ptr->m_cost);
			memcpy(TheBlock,  instance->memory[ij1].v, ARGON2_BLOCK_SIZE);

            blake2b_state BlakeHash3;
			bblake2b_init(&BlakeHash3); //, 32);
			bblake2b_update(&BlakeHash3, (unsigned char*)Y1, sizeof(uint256));
			bblake2b_update(&BlakeHash3, (unsigned char*)TheBlock, ARGON2_BLOCK_SIZE);
			bblake2b_final(&BlakeHash3, (unsigned char*)Y1); 
			}

				uint256 zY1;
				memcpy((unsigned char*)&zY1, (unsigned char*)Y1,32);
				if (zY1 <= hashTarget) {
					return TheNonce;
				}
